...
export default function Dashboard() {
  ...
  const liveCost = stats?.daily?.totalCostUsd || 0;
  const liveTokens = (stats?.daily?.totalInputTokens || 0) + (stats?.daily?.totalOutputTokens || 0);
  const liveSessions = stats?.daily?.totalSessions || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Tokens Today" value={liveTokens.toLocaleString()} sub="Input + Output" icon={Zap} color="#8b5cf6" />
        <StatCard title="Active Sessions" value={liveSessions} sub="All time sessions" icon={Brain} color="#3b82f6" />
        <StatCard title="Total Cost Today" value={`$${liveCost.toFixed(4)}`} sub="USD" icon={DollarSign} color="#10b981" />
        <StatCard title="Memory Index" value={stats?.daily?.totalToolCalls || 0} sub="Tool calls today" icon={Activity} color="#f59e0b" />
      </div>
