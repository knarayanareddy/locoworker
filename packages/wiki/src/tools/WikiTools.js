...
export function makeWikiTools(store, compiler) {
    return [
        {
            name: "wiki_read",
            ...
            async execute(input) {
                ...
                const bySlug = await store.getBySlug(input.query.toLowerCase().replace(/[^a-z0-9]+/g, "-"));
                if (bySlug) {
                ...
                const lines = results.map((r) => `## ${r.page.title}\n${r.page.body.slice(0, 400)}...`);
        ...
        },
        {
            ...
            async execute(input) {
                if (input.title) {
                    ...
                    if (!compiled)
                        return { content: "Compilation failed.", isError: true };
                    return {
        ...
        },
        {
            ...
            async execute() {
                ...
                const lines = pages.map((p) => `• ${p.title} [${p.tags.join(", ") || "untagged"}] — ${p.body ? "compiled" : "pending compile"}`);
        ...
        },
        {
            ...
            async execute(input) {
                ...
                const lines = results.map((r) => `• ${r.page.title} (score: ${r.score}, via: ${r.via})\n  ${r.page.body.slice(0, 200)}`);
                return { content: lines.join("\n\n"), isError: false };
            },
        },
    ];
}
//# sourceMappingURL=WikiTools.js.map