...
export class AutoResearch {
    ...
    constructor(config) {
        ...
        this.queueDir = path.join(MemorySystem.rootFor(config.projectRoot), "research");
    }
    // ── Queue management ───────────────────────────────────────────────────────
    async enqueue(question, priority = 5, tags) {
        ...
        await Bun.write(path.join(this.queueDir, QUEUE_FILE), JSON.stringify(queue, null, 2));
        return id;
    }
    async listQueue() {
    ...
    async runBackgroundPass() {
        ...
        await Bun.write(path.join(this.queueDir, QUEUE_FILE), JSON.stringify(rest, null, 2));
        await this.runJob(entry.id, entry.question, entry.tags);
    }
    // ── Run a single research job ──────────────────────────────────────────────
    async runJob(id, question, tags) {
        const job = {
            id,
            question,
            status: "running",
    ...
    async conductResearch(question) {
    ...
    async persistJob(job) {
        ...
        if (idx >= 0)
            ...
            jobs.push(job);
        await Bun.write(path.join(this.queueDir, JOBS_FILE), JSON.stringify(jobs, null, 2));
    }
}
