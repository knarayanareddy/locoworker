 ...
 */
export async function executeKairosTask(task, config) {
...
async function runDream(config) {
...
async function runDigest(config) {
    ...
    const root = MemorySystem.rootFor(config.projectRoot);
    const transcriptPath = path.join(root, "transcripts", `${today}.md`);
...
async function runResearch(task, config) {
    // @ts-ignore
...
async function runWikiSync(task, config) {
    // @ts-ignore
...
async function runGarbageCollect(config) {
    const root = MemorySystem.rootFor(config.projectRoot);
