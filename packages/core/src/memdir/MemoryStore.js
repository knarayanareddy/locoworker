...
function parseEntry(raw, fallbackType, id) {
    const { frontmatter, body } = parseFrontmatter(raw);
    const type = isMemoryType(String(frontmatter.type ?? "")) ? frontmatter.type : fallbackType;
    return {
        id,
        type,
        ...
        sessionId: frontmatter.sessionId && String(frontmatter.sessionId).length > 0
