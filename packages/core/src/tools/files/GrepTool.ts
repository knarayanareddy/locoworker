...
export const GrepTool: ToolDefinition<GrepInput> = {
  name: "Grep",
  description:
    "Search file contents using ripgrep. Returns matching lines. Falls back to a JS implementation if rg is unavailable.",
  ...
  async execute(input: GrepInput, ctx: ExecutionContext): Promise<ToolResult> {
    ...
    if (input.context !== undefined) args.push(`-C${input.context}`);
