...
export const ReadFileTool: ToolDefinition<ReadInput> = {
  name: "Read",
  description:
    "Read a file from disk. Returns content with line numbers (cat -n style). Use offset/limit for large files.",
  ...
  async execute(input: ReadInput, ctx: ExecutionContext): Promise<ToolResult> {
    ...
    if (!info.isFile()) return err(`Not a file: ${input.path}`);
    if (info.size > MAX_BYTES) {
      return err(
        `File too large (${info.size} bytes, limit ${MAX_BYTES}). Use offset/limit to page.`,
      );
    }
