...
export const GlobTool: ToolDefinition<GlobInput> = {
  name: "Glob",
  description:
    "Find files by pattern (e.g. 'src/**/*.ts'). Returns up to 500 paths sorted by name. Honors .gitignore.",
  ...
  async execute(input: GlobInput, ctx: ExecutionContext): Promise<ToolResult> {
    ...
    for await (const m of glob.scan({ cwd: root, dot: false, absolute: false })) {
      ...
      if (m.includes("node_modules/") || m.includes(".git/")) continue;
      matches.push(m);
      if (matches.length >= MAX_RESULTS) break;
    }

    matches.sort();
    if (matches.length === 0) return ok(`No files match "${input.pattern}"`);
