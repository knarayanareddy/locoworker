...
import type { SlashCommand, SlashOutput } from "./SlashCommand.js";
...
export const memoryListCommand: SlashCommand = {
  ...
  async execute(args, ctx): Promise<SlashOutput> {
    ...
    if (entries.length === 0) return { type: "text", text: "no memories" };
    const text = entries
      .map(
        (e) =>
...
export const memorySearchCommand: SlashCommand = {
  ...
  async execute(args, ctx): Promise<SlashOutput> {
    if (args.length === 0) return { type: "text", text: "usage: /memory-search <query>" };
    const query = args.join(" ");
    const results = await ctx.memory.query(query, 5);
    if (results.length === 0) return { type: "text", text: `no matches for "${query}"` };
    const text = results
      .map(
        (r) =>
