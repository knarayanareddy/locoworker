...
export class ReportGenerator {
    ...
    computeBeliefDistribution(agents) {
        let stronglyFor = 0, mildlyFor = 0, neutral = 0, mildlyAgainst = 0, stronglyAgainst = 0;
        for (const a of agents) {
            if (a.beliefScore >= 0.8)
                stronglyFor++;
            else if (a.beliefScore >= 0.6)
                mildlyFor++;
            else if (a.beliefScore >= 0.4)
                neutral++;
            else if (a.beliefScore >= 0.2)
                mildlyAgainst++;
            else
                stronglyAgainst++;
        }
    ...
    formatMarkdown(data) {
        ...
        return [
            ...
            "",
            "## Top Influencers",
            ...topInfluencers.map((i) => `- **${i.name}** (${i.personality}) — final belief: ${(i.beliefScore * 100).toFixed(0)}%`),
            "",
            "## Key Moments",
            ...keyMoments.map((m) => `- ${m}`),
            "",
