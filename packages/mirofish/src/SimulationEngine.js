...
export class SimulationEngine {
    ...
    async run(onRoundComplete) {
        this.state.status = "running";
        this.state.startedAt = new Date().toISOString();
        try {
            this.state.agents = await this.factory.generateAgents(this.config.seedDocument, this.config.agentCount);
            for (let round = 1; round <= this.config.rounds; round++) {
                ...
                if (onRoundComplete)
                    onRoundComplete(round, roundActions);
            }
    ...
    async simulateRound(round) {
        ...
        for (let i = 0; i < agents.length; i += this.config.concurrency) {
            ...
            const batchActions = await Promise.all(batch.map((agent) => this.agentTurn(agent, round)));
        ...
        for (const action of actions) {
            ...
            if (agent.influenceScore > 0.6) {
                for (const followerId of agent.followedBy) {
                    ...
                    if (follower) {
                        follower.beliefScore = Math.min(1, Math.max(0, follower.beliefScore + action.beliefDelta * 0.3));
                    }
                }
            }
        }
